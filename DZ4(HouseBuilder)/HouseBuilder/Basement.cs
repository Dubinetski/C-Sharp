...
namespace HouseBuilder {
	class Basement : IPart {
		...
		public Basement(int durationItemBuild, decimal costItem, int count) {
			Count = count;
			Cost = costItem * Count;
			FullBuildDuration = durationItemBuild * Count;
			LostBuildDuration = FullBuildDuration;
		}
		public Basement(int durationItemBuild, decimal costItem) : this(durationItemBuild, costItem, 1) { }
		...
		public override string ToString() {
			if (Completing == 0) {
				return String.Format("{0,-10} (not started)", "Basement");
			}
			return String.Format("{0,-10} (complit {1:N1} from {2})","Basement", Completing, Count);
		}
	}
}
